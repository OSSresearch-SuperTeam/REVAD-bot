...
module.exports =
  commit_comment: (adapter, data, callback) ->
    ...
    callback "[#{repo_link}] New comment by #{comment.user.login}
              on commit #{commit_link}: \n
              \"#{comment.body}\""
  ...
  deployment: (adapter, data, callback) ->
    ...
    callback "New deployment #{deploy.id}
              from: #{repo.full_name}
              to: #{deploy.environment} started
              by: #{deploy.creator.login}"
  ...
  deployment_status: (adapter, data, callback) ->
    ...
    callback "Deployment #{deploy.id}
              from: #{repo.full_name}
              to: #{deploy.environment} - #{deploy_status.state}
              by #{deploy.status.creator.login}"
  ...
  gollum: (adapter, data, callback) ->
    ...
    callback "[#{repo_link}] Wiki page: #{page.page_name} #{page.action}
              by #{sender.login}"
  ...
  issue_comment: (adapter, data, callback) ->
    ...
    callback "[#{repo_link}] New comment
              on #{comment_link}
  ...
  membership: (adapter, data, callback) ->
    ...
    callback "#{org.login} #{data.action} #{member.login}
              to #{scope} #{team.name}"
  ...
  page_build: (adapter, data, callback) ->
    ...
    if build?
      if build.status is "built"
        callback "#{build.pusher.login} built #{data.repository.full_name} pages
                  at #{build.commit} in #{build.duration}ms."
      if build.error.message?
        callback "Page build for #{data.repository.full_name}
                  errored: #{build.error.message}."
  ...
  pull_request_review_comment: (adapter, data, callback) ->
    ...
    callback "[#{repo_link}] New comment on Pull Request #{comment_link}
              by #{comment.user.login}: \n\"#{comment.body}\""
